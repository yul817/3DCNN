...
def split_trainingANDtesting(all_filenames):
    test_prop = 0.3
    testing_size = int(test_prop*len(all_filenames))
    dataset = numpy.array(all_filenames) 

    train = list(dataset[:-testing_size])
    test = list(dataset[-testing_size:])

    return train, test


def getFilename(targetpath):
    all_filenames = []
...
def getData(file):
    ...
    feature_and_label = format(feature, file)
...
def main_processing():
    ...
    num_of_loop = 0
    train_filenames, test_filenames = getFilename("/Users/yuelyu/Documents/project/CASP_SCWRL")

    for f in train_filenames:
        feature_and_label = getData(f)   
        num_of_loop += 1
